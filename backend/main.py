...
async def process_bookmarks(data: InputList):
    ...
    for cluster_id, texts in cluster_dict.items():
        ...
        <instructions>
        <primary_rules>
        - 정확히 1-4개의 완전한 명사 어절(띄어쓰기 단위)만 사용
        - 어절을 절대 중간에 끊거나 자르지 않음
        - 한국어로만 작성
        - 모든 구두점(마침표, 쉼표, 따옴표, 괄호 등) 및 특수문자 사용 금지
        - 한 줄로만 출력
        - 길이를 맞추기 위해 단어를 자를 바엔 핵심어만 선택할 것.
        - 카테고리명은 최대 15토큰 이내로 출력
        ...
        올바른 출력: "자연어 처리 기술"
        잘못된 출력: "트랜스포머와 딥러닝을 활용한 자연어 처리" ← 4어절 초과 및 부자연스러움
        </example_1>
        ...
        4. 1-4어절, 15토큰 제한 내에서 가장 적절한 카테고리명을 선택하세요
        </analysis_process>

        <output_format>
        분석 결과로 도출된 카테고리명을 15토큰 이내로만 출력하세요. 추가 설명이나 분석 과정은 포함하지 마세요.
        </output_format>
        </task>
