...
async def process_bookmarks(data: InputList):
    ...
    for cluster_id, texts in cluster_dict.items():
        ...
        - 한국어로만 작성
        - 절대 특수문자 및 구두점을 포함하지 않는다. (예: 마침표 ., 쉼표 ,, 따옴표 ", ', 괄호 (), 물음표 ?, 느낌표 !, 콜론 :, 세미콜론 ;, 대시 -, 밑줄 _, 슬래시 /, \, 앰퍼샌드 &, 별표 *, 퍼센트 %, 달러 기호 $, 해시 #, 골뱅이 @, 캐럿 ^, 물결 ~, 등호 =, 더하기 +, 작은 따옴표 ' ', 큰 따옴표 " ",  백틱 `)
        ...
        <example_2>
        입력 문서들:
        "블록체인 합의 알고리즘: PoW와 PoS의 비교 분석"
        "스마트 컨트랙트 보안: 이더리움 기반 DApp 취약점 분석"
        "분산원장 기술 동향: 하이퍼레저와 코다 플랫폼 비교"

        올바른 출력: "블록체인 기술"
        잘못된 출력: "블록체인" ← 너무 포괄적, 구체성 부족
        </example_2>

        <example_3>
        입력 문서들:
        "머신러닝 모델 해석: SHAP과 LIME을 활용한 XAI 기법"
        "설명가능한 AI 프레임워크: 의료 진단 AI의 투명성 확보"
        "AI 윤리와 신뢰성: 공정성 평가 및 편향 제거 방법"

        올바른 출력: "설명가능한 AI"
        잘못된 출력: "머신러닝 해석 방법론" ← 부자연스러운 조합
        </example_3>
        </examples>
        ...
        <output_format>
        분석 결과로 도출된 카테고리명을 10토큰 정도로 출력하세요. 추가 설명이나 분석 과정은 포함하지 마세요. 절대 특수문자 및 구두점을 포함하지 마시오. (예: 마침표 ., 쉼표 ,, 따옴표 ", ', 괄호 (), 물음표 ?, 느낌표 !, 콜론 :, 세미콜론 ;, 대시 -, 밑줄 _, 슬래시 /, \, 앰퍼샌드 &, 별표 *, 퍼센트 %, 달러 기호 $, 해시 #, 골뱅이 @, 캐럿 ^, 물결 ~, 등호 =, 더하기 +, 작은 따옴표 ' ', 큰 따옴표 " ",  백틱 `)
        </output_format>
        </task>
