...
async def process_bookmarks(data: InputList):
    ...
    # ① PCA (최대 30차원으로 축소) 10
    n_pca = min(10, embeddings_scaled.shape[1] - 1)  # 2차원으로 축소
    if embeddings_scaled.shape[1] > 30:
        embeddings_scaled = PCA(n_components=n_pca, random_state=42).fit_transform(embeddings_scaled)

    # 4) UMAP 차원 축소 5 0 20 
    reducer = umap.UMAP(
        n_neighbors=15,        # 더 가까운 이웃만 고려하여 세밀한 클러스터링
        min_dist=0,         # 클러스터 간 거리를 더 크게 설정하여 더 엄격한 클러스터링
        metric='cosine',
        n_components=20,       # 2차원으로 축소
        init='random',
        random_state=42
    )
